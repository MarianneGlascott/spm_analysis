# =========================================================
# Script title: 04_derive_variables_6.R
...
# Purpose: Derive analysis variables from the analysis-ready
#          MS4 dataset for summaries, plotting, experiment-
...
# Read the analysis-ready dataset from Script 03
# so excluded rows cannot leak back into downstream derivation.
...
# ---------------------------------------------------------
# 4. Import analysis-ready dataset
# ---------------------------------------------------------
...
if ("exclusion_flag" %in% names(dat)) {
  if (any(dat$exclusion_flag, na.rm = TRUE)) {
    stop(
      "Analysis-ready dataset still contains excluded rows. Please review Script 03.",
      call. = FALSE
    )
  }
}

cat("Analysis-ready input confirmed.\n\n")
# ---------------------------------------------------------
...
optional_numeric_cols <- intersect(
  c("lux_exposure", "ntu", "bw_ug_l", "days_from_start"),
  names(dat)
)
...
dat <- dat |>
  dplyr::mutate(
    ...
    log10_bw_ug_l_plus1 = dplyr::if_else(
      "bw_ug_l" %in% names(dat) & !is.na(bw_ug_l),
      log10(bw_ug_l + 1),
      NA_real_
    ),
...
# ---------------------------------------------------------
# 11. Derive plotting, treatment, and Experiment 3 comparison variables
# ---------------------------------------------------------
...
derived_variable_summary <- tibble::tibble(
  ...
  description = c(
    ...
    "log10(bw_ug_l + 1) transform for brake-wear mass-loading analyses",
...
experiment_level_summary <- dat |>
  ...
  dplyr::summarise(
    ...
    min_bw_ug_l = safe_min(bw_ug_l),
    max_bw_ug_l = safe_max(bw_ug_l),
    .groups = "drop"
  ) |>
