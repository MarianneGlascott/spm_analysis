# =========================================================
# Script title: 08_models_exp3_brake_size_v4.R
...
#          comparison models for motile fraction using
#          primary cell counts, generate prediction outputs,
#          compare candidate models, and save experiment-
#          specific summaries and figures to disk.
# Inputs:
...
#   cbind(mobile_cell_count, stationary_cell_count) ~
#   particle_size_class * days_from_start + (1 | culture)
# - Experiment 3 design:
#   Brake wear coarse (BWC) vs brake wear fine (BWF),
#   under equal mass loading rather than NTU-matched exposure.
# - NTU is not the primary predictor in this experiment.
# - mass_loading_ug_l may be explored as a sensitivity term
#   if variation exists within this experiment.
# =========================================================
...
cat("Days:", paste(sort(unique(exp3_dat$days_from_start)), collapse = ", "), "\n\n")

# ---------------------------------------------------------
# 7. Standardise size class and derive support vars
# ---------------------------------------------------------
...
if (!"size_class" %in% names(exp3_dat)) {
  stop("Column 'size_class' is required for Experiment 3.", call. = FALSE)
}

exp3_dat <- exp3_dat |>
  dplyr::mutate(
    particle_size_class = standardise_upper_trim(size_class),
    ...
    days_from_start_c = as.numeric(scale(days_from_start, scale = FALSE)),
...
if ("bw_ug_l" %in% names(exp3_dat)) {
  exp3_dat <- exp3_dat |>
    dplyr::mutate(
      bw_ug_l = suppressWarnings(as.numeric(bw_ug_l)),
      log10_bw_ug_l_plus1 = dplyr::if_else(
        !is.na(bw_ug_l),
        log10(bw_ug_l + 1),
        NA_real_
      )
...
has_bw_ug_l <- "bw_ug_l" %in% names(exp3_dat)

exp3_dataset_summary <- exp3_dat |>
  dplyr::group_by(days_from_start, particle_size_class) |>
  dplyr::summarise(
    n_rows = dplyr::n(),
    ...
    mean_bw_ug_l = if (has_bw_ug_l && any(!is.na(mass_loading_ug_l))) {
      mean(mass_loading_ug_l, na.rm = TRUE)
    } else {
      NA_real_
    },
    min_bw_ug_l = if (has_has_bw_ug_l && any(!is.na_bw_ug_l))) {
      min(mass_loading_ug_l, na.rm = TRUE)
    } else {
      NA_real_
    },
    max_bw_ug_l = if (has_bw_ug_l && any(!is.na(bw_ug_l))) {
      max(mass_loading_ug_l, na.rm = TRUE)
    } else {
      NA_real_
    },
    .groups = "drop"
  ) |>
  dplyr::arrange(days_from_start, particle_size_class)
...
if (!"log10_bw_ug_l_plus1" %in% names(exp3_dat)) {
  stop("log10_bw_ug_l_plus1 not found. Run Script 04 again.", call. = FALSE)
}

if (all(is.na(exp3_dat$log10_bw_ug_l_plus1))) {
  stop("All bw_ug_l values are NA. Check upstream data mapping.", call. = FALSE)
}
...
formula_full_primary <- cbind(mobile_cell_count, stationary_cell_count) ~
  particle_size_class * log10_bw_ug_l_plus1 * days_from_start_c +
  (1 | culture)
...
mass_loading_varies <- FALSE

if ("bw_ug_l" %in% names(exp3_dat)) {
  unique_mass_values <- unique(stats::na.omit(exp3_dat$bw_ug_l))
  bw_ug_like _varies <- length(unique_mass_values) > 1
}

formula_mass_sensitivity <- NULL
if (mass_loading_varies) {
  formula_mass_sensitivity <- cbind(mobile_cell_count, stationary_cell_count) ~
    particle_size_class * days_from_start + log10_bw_ug_l_plus1 + (1 | culture)
}

cat("Candidate model formulas defined.\n")
cat("bw_ug_l sensitivity model included:", bw_ug_l_varies, "\n\n")
...
fit_full_primary <- fit_check_save_model(
  ...
  data = exp3_dat,
  model_name = "exp3_brake_size_full_primary",
...
fit_additive_primary <- fit_check_save_model(
  ...
  data = exp3_dat,
  model_name = "exp3_brake_size_additive_primary",
  model_subdir = exp3_model_subdir,
  save_terms_csv = TRUE,
  save_meta_csv = TRUE,
  save_diagnostics = TRUE,
...
fit_null <- fit_check_save_model(
  ...
  data = exp3_dat,
  model_name = "exp3_brake_size_null",
  model_subdir = exp3_model_subdir,
  save_terms_csv = TRUE,
  save_meta_csv = TRUE,
  save_diagnostics = TRUE,
...
fit_bw_ug_l_sensitivity <- NULL
if (bw_ug_l_varies && !is.null(formula_mass_sensitivity)) {
  fit_mass_sensitivity <- fit_check_save_model(
    formula = formula_mass_sensitivity,
    data = exp3_dat,
    model_name = "exp3_brake_size_bw_ug_l_sensitivity",
    model_subdir = exp3_model_subdir,
    save_terms_csv = TRUE,
    save_meta_csv = TRUE,
    save_diagnostics = TRUE,
...
if (!all(c("fit_full_primary", "fit_additive_primary", "fit_null") %in% ls())) {
  stop("One or more Experiment 3 model fits failed. Check errors above.", call. = FALSE)
}
...
lrt_full_vs_mass_sensitivity <- NULL
if (!is.null(fit_mass_sensitivity)) {
  lrt_full_vs_mass_sensitivity <- tryCatch(
    stats::anova(fit_full_primary$model, fit_mass_sensitivity$model),
    error = function(e) e
  )
}
...
preferred_model <- switch(
  ...
  full_primary = fit_full_primary$model,
  additive_primary = fit_additive_primary$model,
  mass_loading_sensitivity = if (!is.null(fit_mass_sensitivity)) fit_mass_sensitivity$model else fit_full_primary$model,
  null = fit_null$model,
...
pred_at <- list(
  ...
  days_from_start = sort(unique(exp3_dat$days_from_start))
)

if (preferred_model_name == "mass_loading_sensitivity" && "log10_mass_loading_ug_l_plus1" %in% names(exp3_dat)) {
  pred_at$log10_mass_loading_ug_l_plus1 <- stats::median(
    exp3_dat$log10_mass_loading_ug_l_plus1,
    na.rm = TRUE
  )
}
...
pred_grid <- make_prediction_grid(
  data = exp3_dat,
  focal_terms = c("particle_size_class", "days_from_start"),
  at = pred_at,
  n_numeric = 100
)
...
pred_out <- predict_glmmtmb_response(
  ...
  dplyr::mutate(
    day_label = paste0("Day ", days_from_start),
    prediction_scale = "particle_size_class"
  )
...
p_exp3 <- ggplot2::ggplot() +
  ...
  ggplot2::labs(
    ...
    subtitle = "Model-predicted motile fraction for coarse versus fine fractions",
    x = "Particle size class",
    y = "Motile fraction",
    colour = "Particle class",
    caption = "Points show raw observations; pointranges show model predictions with 95% confidence intervals. Panels are shown by day from start."
  )

# ---------------------------------------------------------
# 17. Save Experiment 3 figure
# ---------------------------------------------------------
...
if (exists("write_figure_caption_md", mode = "function", inherits = TRUE)) {
  write_figure_caption_md(
    figure_name = "Fig4_brake_size_model_predictions",
    caption_text = "Model-predicted motile fraction for brake-wear coarse and fine fractions in the equal-mass Experiment 3 comparison. Raw observations are overlaid, 95% confidence intervals are shown, and panels are facetted by day from start.",
    subdir = exp3_figure_subdir
  )
}
...
if (exists("write_model_summary_rds", mode = "function", inherits = TRUE)) {
  write_model_summary_rds(
    object = list(
      ...
      lrt_full_vs_mass_sensitivity = lrt_full_vs_mass_sensitivity,
      preferred_model_name = preferred_model_name,
      mass_loading_varies = mass_loading_varies
    ),
...
cat("Mass loading varies within experiment:\n")
cat(mass_loading_varies, "\n\n")
...
if (!is.null(lrt_full_vs_mass_sensitivity)) {
  cat("Likelihood ratio test: full vs mass-loading sensitivity\n")
  print(lrt_full_vs_mass_sensitivity)
  cat("\n")
}
