# =========================================================
# Script title: 08_models_exp3_brake_size.R
...
if ("mass_loading_ug_l" %in% names(exp3_dat)) {
  exp3_dat <- exp3_dat |>
    dplyr::mutate(
      mass_loading_ug_l = suppressWarnings(as.numeric(mass_loading_ug_l)),
...
has_mass_loading <- "mass_loading_ug_l" %in% names(exp3_dat)
...
exp3_dataset_summary <- exp3_dat |>
  ...
  dplyr::summarise(
    ...
    mean_mass_loading_ug_l = if (has_mass_loading && any(!is.na(mass_loading_ug_l))) {
      mean(mass_loading_ug_l, na.rm = TRUE)
    } else {
      NA_real_
    },
    min_mass_loading_ug_l = if (has_mass_loading && any(!is.na(mass_loading_ug_l))) {
      min(mass_loading_ug_l, na.rm = TRUE)
    } else {
      NA_real_
    },
    max_mass_loading_ug_l = if (has_mass_loading && any(!is.na(mass_loading_ug_l))) {
...
formula_full_primary <- cbind(mobile_cell_count, stationary_cell_count) ~
  pparticle_size_class * days_from_start_c + (1 | culture)

formula_additive_primary <- cbind(mobile_cell_count, stationary_cell_count) ~
  particle_size_class * days_from_start_c + (1 | culture)
...
if ("mass_loading_ug_l" %in% names(exp3_dat)) {
  unique_mass_values <- unique(stats::na.omit(exp3_dat$mass_loading_ug_l))
  mass_loading_varies <- length(unique_mass_values) > 1
}
...
if (mass_loading_varies) {
  formula_mass_sensitivity <- cbind(mobile_cell_count, stationary_cell_count) ~
    particle_size_class * days_from_start + log10_mass_loading_ug_l_plus1 + (1 | culture)
}

cat("Candidate model formulas defined.\n")
cat("Mass-loading sensitivity model included:", mass_loading_varies, "\n\n")
...
fit_mass_sensitivity <- NULL
if (mass_loading_varies && !is.null(formula_mass_sensitivity)) {
  fit_mass_sensitivity <- fit_check_save_model(
    formula = formula_mass_sensitivity,
    data = exp3_dat,
    model_name = "exp3_brake_size_mass_loading_sensitivity",
    model_subdir = exp3_model_subdir,
    save_terms_csv = TRUE,
    save_meta_csv = TRUE,
    save_diagnostics = TRUE,
