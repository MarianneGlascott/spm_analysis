# =========================================================
# Script title: 08_models_exp3_brake_size_v5.R
...
#          counts, compare candidate models, generate
...
pred_grid <- make_prediction_grid(
  data = exp3_dat,
  focal_terms = c("particle_size_class", "days_from_start", "log10_bw_ug_l_plus1"),
  at = pred_at,
  n_numeric = 100
)
...
pred_out <- predict_glmmtmb_response(
  ...
  dplyr::mutate(
    day_label = paste0("Day ", days_from_start),
    prediction_scale = "particle_size_class_at_median_bw_ug_l"
  )
...
if (exists("write_figure_caption_md", mode = "function", inherits = TRUE)) {
  write_figure_caption_md(
    figure_name = "Fig4_brake_size_model_predictions",
    caption_text = "Model-predicted motile fraction for brake-wear coarse and fine fractions in Experiment 3 under controlled mass loading. Raw observations are overlaid, 95% confidence intervals are shown, and panels are facetted by day from start. Predictions are evaluated at the median brake-wear concentration.",
    subdir = exp3_figure_subdir
  )
}
