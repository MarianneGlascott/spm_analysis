...
raw_8_2 <- read_csv("C:/SPM_Analysis/data_raw/All_data_cleaned_8_2.csv") %>%
  clean_names()

# Sanity check: do we have the columns we need?
raw_8_2 %>% select(matches("mobile|stationary|ntu|days_from_start|experiment", ignore.case = TRUE)) %>% glimpse()

raw_8_2 <- raw_8_2 %>%
  mutate(
    total_cells = mobile_cell_count + stationary_cell_count,
    motile_fraction_calc = if_else(total_cells > 0, mobile_cell_count / total_cells, NA_real_),
    days_from_start = as.factor(days_from_start)  # treat as factor for plotting/faceting
  )
# 1.2 Checks and balances
# Any zero/NA totals?
raw_8_2 %>% summarise(
  n = n(),
  n_total0 = sum(total_cells == 0, na.rm = TRUE),
  n_totalNA = sum(is.na(total_cells)),
  n_motileNA = sum(is.na(motile_fraction_calc))
)

# Are counts ever negative or weird?
raw_8_2 %>%
  summarise(
    min_mobile = min(mobile_cell_count, na.rm = TRUE),
    min_stationary = min(stationary_cell_count, na.rm = TRUE),
    min_total = min(total_cells, na.rm = TRUE)
  )
# 1.3 motility ratio versus calculated figure

raw_8_2 %>%
  select(motility_ratio, motile_fraction_calc) %>%
  summarise(
    n = sum(!is.na(motility_ratio) & !is.na(motile_fraction_calc)),
    max_abs_diff = max(abs(motility_ratio - motile_fraction_calc), na.rm = TRUE),
    mean_abs_diff = mean(abs(motility_ratio - motile_fraction_calc), na.rm = TRUE)
  )

# 1.4 bullet proof that only experiment 8.2 data is included
df_8 <- df_8 %>%
  mutate(
    ntu_plot = log10(ntu + 1)
  )

ggplot(df_8, aes(x = ntu_plot, y = motile_fraction_calc)) +
  geom_point(alpha = 0.5, position = position_jitter(width = 0.02, height = 0)) +
  facet_wrap(~days_from_start) +
...
p_check <- ggplot(df_8, aes(x = ntu_plot, y = motile_fraction_calc)) +
...
# save figure
save_pub_figure(p_check, paste0("NTU_vs_motility_by_day_", Sys.Date()))
