# =========================================================
# Script title: 08_models_exp3_brake_size_v6.R
...
#          gradient, compare candidate models, generate
#          prediction outputs, and save experiment-specific
...
if (all(is.na(exp3_dat$log10_bw_ug_l_plus1))) {
  stop("All log10_bw_ug_l_plus1 values are NA. Check upstream data mapping.", call. = FALSE)
}

cat("Experiment 3 modelling support variables derived.\n")
cat("Size classes observed:", paste(observed_size_levels, collapse = ", "), "\n")
cat("Brake-wear concentration range (ug/L):",
    paste0(
...
exp3_dataset_summary <- exp3_dat |>
  ...
  dplyr::summarise(
    ...
    mean_bw_ug_l = mean(bw_ug_l, na.rm = TRUE),
...
fit_partial_primary <- fit_check_save_model(
  ...
  data = exp3_dat,
  model_name = "exp3_brake_size_partial_primary",
  model_subdir = exp3_model_subdir,
  save_terms_csv = TRUE,
  save_meta_csv = TRUE,
  save_diagnostics = TRUE,
...
missing_models <- required_models[!vapply(required_models, exists, logical(1))]
...
lrt_partial_vs_full <- tryCatch(
  stats::anova(fit_partial_primary$model, fit_full_primary$model),
  error = function(e) e
)
...
if (preferred_model_name %in% c("full_primary", "partial_primary", "additive_primary")) {
  pred_grid <- make_prediction_grid(
    data = exp3_dat,
    focal_terms = c("log10_bw_ug_l_plus1", "particle_size_class", "days_from_start"),
    at = list(
  ...
  pred_grid <- make_prediction_grid(
    ...
    at = list(
      particle_size_class = levels(droplevels(exp3_dat$particle_size_class)),
      days_from_start = sort(unique(exp3_dat$days_from_start)),
      log10_bw_ug_l_plus1 = stats::median(exp3_dat$log10_bw_ug_l_plus1, na.rm = TRUE)
    ),
    n_numeric = 100
  )
  ...
  pred_out <- predict_glmmtmb_response(
    ...
    dplyr::mutate(
      bw_ug_l_backtransformed = (10^log10_bw_ug_l_plus1) - 1,
      day_label = paste0("Day ", days_from_start),
      prediction_scale = "null_model_at_median_bw_ug_l"
    )
}
...
if (preferred_model_name %in% c("full_primary", "partial_primary", "additive_primary")) {
  p_exp3 <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      ...
      subtitle = "Model-predicted motile fraction versus log10(bw_ug_l + 1), by size class and day",
  ...
  p_exp3 <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      title = "Brake-wear size comparison under controlled mass loading",
      subtitle = "Model-predicted motile fraction for coarse versus fine fractions at median bw_ug_l",
      x = "Particle size class",
      y = "Motile fraction",
      colour = "Particle class",
      caption = "Points show raw observations; pointranges show model predictions with 95% confidence intervals. Panels are shown by day from start; predictions are evaluated at the median brake-wear concentration."
    )
}
...
if (exists("write_figure_caption_md", mode = "function", inherits = TRUE)) {
  write_figure_caption_md(
    figure_name = "Fig4_brake_size_model_predictions",
    caption_text = if (preferred_model_name %in% c("full_primary", "partial_primary", "additive_primary")) {
      "Model-predicted motile fraction as a function of log10(brake-wear concentration + 1) in the Experiment 3 brake-wear size comparison. Predictions are coloured by brake-wear size class, raw observations are overlaid, and 95% confidence intervals are shown. Panels are shown by day from start."
    } else {
      "Model-predicted motile fraction for brake-wear coarse and fine fractions in Experiment 3 under controlled mass loading. Raw observations are overlaid, 95% confidence intervals are shown, and panels are facetted by day from start. Predictions are evaluated at the median brake-wear concentration."
    },
    subdir = exp3_figure_subdir
  )
}
