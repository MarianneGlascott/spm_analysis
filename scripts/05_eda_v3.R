...
# - outputs/tables/05_eda_exp3_brake_presence_summary.csv
...
# - EDA operates within the Manuscript 4 experiment block.
# - Primary grouping is based on experiment / experiment_num.
...
file_exp3_presence_summary <- file.path(dir_tables, "05_eda_exp3_brake_presence_summary.csv")
...
dat <- dat |>
  dplyr::mutate(
    experiment_plot_label = dplyr::case_when(
      ...
      experiment_num == 9.2 ~ "Experiment 1: Light-only gradient",
      experiment_num == 10.2 ~ "Experiment 2: Defined particle concentration series",
      experiment_num == 11.2 ~ "Experiment 3: Particle size comparison under equal mass loading",
      experiment_num == 8.2 ~ "Experiment 4: Field-derived SPM gradient",
...
dataset_overview <- tibble::tibble(
  ...
  value = c(
    ...
    if (has_video_file) safe_n_distinct(dat$video_file) else NA_integer_,
...
exp3_size_dat <- exp3_dat |>
  dplyr::filter(!is.na(brake_size_treatment))

exp3_group_vars <- intersect(c("days_from_start", "brake_size_treatment", "size_class_plot", "mass_loading_ug_l"), names(exp3_size_dat))
exp3_summary <- exp3_size_dat |>
  ...
  dplyr::arrange(days_from_start, brake_size_treatment, size_class_plot, mass_loading_ug_l)

exp3_presence_dat <- exp3_dat |>
  dplyr::filter(!is.na(brake_exposure))

exp3_presence_group_vars <- intersect(c("days_from_start", "brake_exposure"), names(exp3_presence_dat))
exp3_presence_summary <- exp3_presence_dat |>
  dplyr::group_by(dplyr::across(dplyr::all_of(exp3_presence_group_vars))) |>
  dplyr::summarise(
    n_rows = dplyr::n(),
    n_cultures = safe_n_distinct(culture),
    n_wells = safe_n_distinct(well),
  ...
  dplyr::arrange(days_from_start, brake_exposure)
...
readr::write_csv(exp3_presence_summary, file_exp3_presence_summary)
...
if (nrow(exp3_size_dat) > 0 && "size_class_plot" %in% names(exp3_size_dat)) {
...
cat("Experiment 3 size summary:\n")
print(exp3_summary)
cat("\n")

cat("Experiment 3 brake-presence summary:\n")
print(exp3_presence_summary)
cat("\n")
...
cat("Experiment 3 size rows by toxin_exposure:\n")
if ("brake_size_treatment" %in% names(dat)) {
  print(table(droplevels(as.factor(dat$toxin_exposure[!is.na(dat$brake_size_treatment)])), useNA = "ifany"))
}
cat("\n")

cat("Experiment 3 brake-presence rows by toxin_exposure:\n")
if ("brake_exposure" %in% names(dat)) {
  print(table(droplevels(as.factor(dat$toxin_exposure[!is.na(dat$brake_exposure)])), useNA = "ifany"))
}
cat("\n")
