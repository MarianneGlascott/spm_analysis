# =========================================================
# Script title: 08_models_exp3_brake_size_v8.R
...
if ("use_exp3_brake_size" %in% names(exp3_dat)) {
  exp3_dat <- exp3_dat |>
    dplyr::filter(use_exp3_brake_size %in% TRUE)
}

if (nrow(exp3_dat) == 0) {
  stop(
    "No Experiment 3 rows remain after experiment/use_exp3_brake_size filtering.",
    call. = FALSE
  )
}
...
if (nrow(exp3_dat) == 0) {
  stop(
    "No Experiment 3 rows remain after removing missing response/predictor values.",
    call. = FALSE
  )
}
...
n_size_levels <- dplyr::n_distinct(exp3_dat$particle_size_class, na.rm = TRUE)

if (n_size_levels < 2) {
  stop(
    "Experiment 3 modelling requires at least two observed particle_size_class levels (BWC and BWF).",
    call. = FALSE
  )
}

if (sum(!is.na(exp3_dat$log10_bw_ug_l_plus1)) < 2) {
  stop(
    "Experiment 3 modelling requires non-missing brake-wear concentration values.",
    call. = FALSE
  )
}

observed_size_levels <- levels(droplevels(exp3_dat$particle_size_class))
observed_days <- sort(unique(stats::na.omit(exp3_dat$days_from_start)))
...
cat(
  ...
  paste0(
    round(safe_min(exp3_dat$bw_ug_l), 3),
    " to ",
    round(safe_max(exp3_dat$bw_ug_l), 3)
  ),
  "\n"
)
...
if (!all(is.na(exp3_dat$days_from_start))) {
