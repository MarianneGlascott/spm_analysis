  ...
  dplyr::mutate(
    ...
    particle_type = dplyr::case_when(
      experiment_type == "DEFINED_PARTICLES" &
        as.character(toxin_exposure) %in% c("SAND", "KAOLINITE", "PEAT") ~ as.character(toxin_exposure),
      experiment_type == "FIELD_SPM" &
        as.character(toxin_exposure) == "SPM" ~ "SPM",
      experiment_type == "BRAKE_SIZE" &
        as.character(toxin_exposure) %in% c("BWC", "BWF") ~ "BRAKE_WEAR",
    ...
    size_class = dplyr::case_when(
      experiment_type == "BRAKE_SIZE" &
        as.character(toxin_exposure) == "BWC" ~ "COARSE",
      experiment_type == "BRAKE_SIZE" &
        as.character(toxin_exposure) == "BWF" ~ "FINE",
...
# ---------------------------------------------------------
# 13b. Concentration variables: retain source values only
# ---------------------------------------------------------

# bw_ug_l and cu_ug_l are separate source variables.
# Values of 0 indicate that no appropriate concentration
# applies for that row. No backfilling or cross-variable
...
cat("bw_ug_l and cu_ug_l retained as separate source variables with no backfilling.\n")
