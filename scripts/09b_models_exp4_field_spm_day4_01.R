# =========================================================
# Script title: 09b_models_exp4_field_spm_day4_01.R
...
# Purpose: Fit Day 4-only Experiment 4 field-derived SPM
#          models for kelp zoospore motility using primary
#          count data.
#
# Inputs:
...
#
# Outputs:
# - outputs/models/day4/exp4_field_spm_day4/*.rds
# - outputs/models/day4/exp4_field_spm_day4/*.csv
# - outputs/tables/day4/09b_exp4_field_spm_day4_dataset_summary.csv
# - outputs/tables/day4/09b_exp4_field_spm_day4_model_comparison.csv
# - outputs/tables/day4/09b_exp4_field_spm_day4_preferred_model_terms.csv
...
# - outputs/figures/day4/Fig7_exp4_field_spm_day4_model_predictions.{pdf,png,tiff}
# - outputs/logs/day4/09b_exp4_field_spm_day4_log_*.txt
#
# Notes:
...
# - Candidate models explicitly compare monotonic, nonlinear,
#   treatment-level factor, and null responses.
# - Main Figure 8 reference condition:
#   lowest observed NTU in the Day 4 field-SPM dataset.
# =========================================================

cat("\n========================================================\n")
cat("SCRIPT 09b: MODELS EXP4 FIELD SPM DAY 4\n")
...
file_dataset_summary <- file.path(
  dir_day4_tables,
  "09b_exp4_field_spm_day4_dataset_summary.csv"
)

file_model_comparison <- file.path(
  dir_day4_tables,
  "09b_exp4_field_spm_day4_model_comparison.csv"
)

file_model_terms <- file.path(
  dir_day4_tables,
  "09b_exp4_field_spm_day4_preferred_model_terms.csv"
)

file_predictions <- file.path(
  dir_day4_tables,
  "09b_exp4_field_spm_day4_predictions.csv"
)
...
file_preferred_model_rds <- file.path(
  dir_day4_models,
  "exp4_field_spm_day4_preferred_model.rds"
)
...
file_predictions_models <- file.path(
  dir_day4_models,
  "exp4_field_spm_day4_predictions.csv"
)

file_relative_effects_models <- file.path(
  dir_day4_models,
  "exp4_field_spm_day4_relative_effects.csv"
)
...
file_log <- file.path(
  dir_day4_logs,
  paste0("09b_exp4_field_spm_day4_log_", timestamp_now, ".txt")
)
...
# ---------------------------------------------------------
# 8. Restrict to Experiment 4: field-derived SPM, Day 4
# ---------------------------------------------------------
...
if (nrow(exp4_dat) == 0) {
  stop("No valid Day 4 Experiment 4 rows found.", call. = FALSE)
}
...
exp4_dat <- exp4_dat |>
  dplyr::mutate(
    ...
    ntu_f = factor(
      ntu,
      levels = sort(unique(ntu)),
      labels = paste0(sort(unique(ntu)), " NTU")
    ),
    culture = as.factor(culture),
    well = as.factor(well),
    experiment_label = "Experiment 4: Field-derived SPM gradient",
    analysis_branch = "day4"
  )

n_ntu_levels <- safe_n_distinct(exp4_dat$ntu)
if (n_ntu_levels < 2) {
  stop(
    "Experiment 4 requires at least two NTU levels for gradient modelling.",
    call. = FALSE
  )
}

cat("Experiment 4 Day 4 subset created.\n")
cat("Rows:", nrow(exp4_dat), "\n")
cat("NTU range:", paste0(safe_min(exp4_dat$ntu), " to ", safe_max(exp4_dat$ntu)), "\n")
cat("NTU levels:", paste(sort(unique(exp4_dat$ntu)), collapse = ", "), "\n")
cat("Culture levels:", safe_n_distinct(exp4_dat$culture), "\n")
cat("Wells:", safe_n_distinct(exp4_dat$well), "\n\n")
...
dataset_summary <- exp4_dat |>
  dplyr::group_by(ntu, ntu_f) |>
  dplyr::summarise(
    n_rows = dplyr::n(),
...
formula_monotonic <- cbind(mobile_cell_count, stationary_cell_count) ~
  log10_ntu_plus1

formula_nonlinear <- cbind(mobile_cell_count, stationary_cell_count) ~
  log10_ntu_plus1 + I(log10_ntu_plus1^2)

formula_factor <- cbind(mobile_cell_count, stationary_cell_count) ~
  ntu_f

formula_null <- cbind(mobile_cell_count, stationary_cell_count) ~ 1

fit_monotonic_betabinomial <- fit_model_safely(
  formula = formula_monotonic,
  data = exp4_dat,
  family = glmmTMB::betabinomial(link = "logit"),
  model_label = "monotonic_betabinomial"
)

fit_nonlinear_betabinomial <- fit_model_safely(
  formula = formula_nonlinear,
  data = exp4_dat,
  family = glmmTMB::betabinomial(link = "logit"),
  model_label = "nonlinear_betabinomial"
)
...
fit_monotonic_binomial <- fit_model_safely(
  formula = formula_monotonic,
  data = exp4_dat,
  family = stats::binomial(link = "logit"),
  model_label = "monotonic_binomial"
)

fit_nonlinear_binomial <- fit_model_safely(
  formula = formula_nonlinear,
  data = exp4_dat,
  family = stats::binomial(link = "logit"),
  model_label = "nonlinear_binomial"
)
...
candidate_models <- list(
  monotonic_betabinomial = list(
    object = fit_monotonic_betabinomial$model_object[[1]],
    family = "betabinomial",
    status = fit_monotonic_betabinomial$model_status[[1]],
    error = fit_monotonic_betabinomial$error_message[[1]]
  ),
  nonlinear_betabinomial = list(
    object = fit_nonlinear_betabinomial$model_object[[1]],
    family = "betabinomial",
    status = fit_nonlinear_betabinomial$model_status[[1]],
    error = fit_nonlinear_betabinomial$error_message[[1]]
  ),
  ...
  monotonic_binomial = list(
    object = fit_monotonic_binomial$model_object[[1]],
    family = "binomial",
    status = fit_monotonic_binomial$model_status[[1]],
    error = fit_monotonic_binomial$error_message[[1]]
  ),
  nonlinear_binomial = list(
    object = fit_nonlinear_binomial$model_object[[1]],
    family = "binomial",
    status = fit_nonlinear_binomial$model_status[[1]],
    error = fit_nonlinear_binomial$error_message[[1]]
  ),
...
min_observed_ntu <- min(exp4_dat$ntu, na.rm = TRUE)
max_observed_ntu <- max(exp4_dat$ntu, na.rm = TRUE)

observed_ntu_values <- sort(unique(exp4_dat$ntu))

# Continuous grid for monotonic/nonlinear/null models.
continuous_ntu_grid <- seq(
  from = min_observed_ntu,
  to = max_observed_ntu,
  length.out = 150
)

continuous_prediction_grid <- tibble::tibble(
  ntu = continuous_ntu_grid,
  log10_ntu_plus1 = log10(ntu + 1),
  ntu_sq = log10_ntu_plus1^2
)

# Include an ntu_f column for robustness; it will not be used
# by continuous models.
continuous_prediction_grid$ntu_f <- factor(
  continuous_prediction_grid$ntu,
  levels = observed_ntu_values,
  labels = paste0(observed_ntu_values, " NTU")
)

# Factor-grid predictions at observed levels only.
factor_prediction_grid <- tibble::tibble(
  ntu = observed_ntu_values,
  log10_ntu_plus1 = log10(ntu + 1),
  ntu_sq = log10_ntu_plus1^2,
  ntu_f = factor(
    ntu,
...
if (preferred_model_name %in% c("factor_betabinomial", "factor_binomial")) {
  prediction_grid <- factor_prediction_grid
  prediction_type <- "observed_ntu_factor_levels"
} else {
  prediction_grid <- continuous_prediction_grid
  prediction_type <- "continuous_ntu_gradient"
}
...
predictions <- predict_response_with_ci(
  ...
  dplyr::mutate(
    experiment = "Experiment 4",
    experiment_num = 8.2,
    experiment_label = "Field-derived SPM gradient",
    analysis_branch = "day4",
    model = preferred_model_name,
    response = "motile_fraction",
    prediction_type = prediction_type,
    reference_condition = paste0("Lowest observed NTU: ", min_observed_ntu),
    treatment_condition = paste0(round(ntu, 3), " NTU")
  ) |>
  dplyr::arrange(ntu)
...
print(utils::head(predictions, 12))

# ---------------------------------------------------------
# 14. Relative effects versus lowest observed NTU
# ---------------------------------------------------------

reference_ntu <- min_observed_ntu

# For relative effects, use biologically interpretable targets
# within the observed range, plus the observed maximum as an
# optional extreme endpoint.
target_ntu_values <- c(25, 100, 400, 500)

target_ntu_values <- target_ntu_values[
  target_ntu_values >= min_observed_ntu &
    target_ntu_values <= max_observed_ntu
]

target_ntu_values <- sort(unique(c(target_ntu_values, max_observed_ntu)))

# Include the reference if it is not already included.
relative_ntu_grid_values <- sort(unique(c(reference_ntu, target_ntu_values)))

if (preferred_model_name %in% c("factor_betabinomial", "factor_binomial")) {
  # For factor models, predictions are only valid at observed NTU levels.
  relative_ntu_grid_values <- relative_ntu_grid_values[
    relative_ntu_grid_values %in% observed_ntu_values
  ]
  
  if (!reference_ntu %in% relative_ntu_grid_values) {
    relative_ntu_grid_values <- sort(unique(c(reference_ntu, relative_ntu_grid_values)))
  }
  
  relative_grid <- tibble::tibble(
    ntu = relative_ntu_grid_values,
    log10_ntu_plus1 = log10(ntu + 1),
    ntu_sq = log10_ntu_plus1^2,
    ntu_f = factor(
      ntu,
...
relative_predictions <- predict_response_with_ci(
  model = preferred_model,
  newdata = relative_grid,
  conf_level = 0.95
) |>
  dplyr::mutate(
    experiment = "Experiment 4",
...
reference_prediction <- relative_predictions |>
  dplyr::filter(ntu == reference_ntu) |>
  dplyr::slice(1)

if (nrow(reference_prediction) != 1) {
  stop("Could not identify the lowest-NTU reference prediction.", call. = FALSE)
}

reference_condition_label <- paste0("Lowest observed NTU: ", reference_ntu)

relative_effects <- relative_predictions |>
  dplyr::filter(ntu != reference_ntu) |>
  dplyr::mutate(
...
# ---------------------------------------------------------
# 15. Figure 7: Experiment 4 Day 4 predictions
# ---------------------------------------------------------
...
if (preferred_model_name %in% c("factor_betabinomial", "factor_binomial")) {
  ...
  p_exp4 <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      ...
      x = "Field-derived SPM treatment",
      y = "Motile fraction",
      caption = paste(
        "Points show raw well-level observations.",
        "Model predictions are shown at observed NTU treatment levels.",
        "Reference for proportional effects: lowest observed NTU."
      )
    )
...
figure_manifest <- save_plot_multi_local(
  plot = p_exp4,
  file_stem = "Fig7_exp4_field_spm_day4_model_predictions",
  width = 7.8,
  height = 5.8,
  dpi = 600
)
...
sink(file_log)
cat("Experiment 4 Day 4 model log\n")
cat("Generated:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n\n")
cat("Input source:\n")
cat(input_source_used, "\n\n")
cat("Rows in Experiment 4 Day 4 dataset:", nrow(exp4_dat), "\n")
cat("NTU range:", safe_min(exp4_dat$ntu), "to", safe_max(exp4_dat$ntu), "\n")
cat("NTU levels:\n")
print(sort(unique(exp4_dat$ntu)))
cat("\nCultures:", safe_n_distinct(exp4_dat$culture), "\n")
cat("Wells:", safe_n_distinct(exp4_dat$well), "\n\n")
