...
# emmeans can output CI columns as lower.CL/upper.CL OR asymp.LCL/asymp.UCL etc.
emm_to_df <- function(emm_obj) {
  s <- as.data.frame(summary(emm_obj, type = "response", infer = c(TRUE, TRUE)))
  
  # Estimate column name (most common for type="response")
  est_col <- intersect(c("response", "prob", "rate", "emmean"), names(s))[1]
  if (is.na(est_col)) {
    stop("Could not find an estimate column in emmeans output. Columns were: ",
         paste(names(s), collapse = ", "))
  }
  
  # CI column name variants
  ...
  if (is.na(lcl_col) || is.na(ucl_col)) {
    stop("Could not find CI columns in emmeans output. Columns were: ",
         paste(names(s), collapse = ", "))
  }
...
emm_ss_df <- emm_to_df(emm_ss) %>%
  filter(is.finite(estimate), is.finite(lcl), is.finite(ucl))
